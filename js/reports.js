...
export const relatorios = {
  ...
  atualizarGraficos: function () {
    ...
    const ganhosDia = ganhosUsuario.filter(g => g.data === hoje).reduce((s, g) => s + g.valor, 0);
    ...
    this[chartVarName] = new Chart(ctx, {
      ...
      options: {
        ...
        plugins: {
          ...
          tooltip: {
            callbacks: { label: (c) => `${c.label}: ${formatarMoeda(c.parsed)}` },
          },
        },
      },
    });
  ...
  setupFerramentas: function () {
    ...
    if ($("atalhoExportar")) $("atalhoExportar").onclick = this.exportarRelatorioCSV.bind(this);
    ...
    if ($("btnCompartilhar")) {
      $("btnCompartilhar").addEventListener("click", () => this.compartilharGanhosFiltrados());
    }
  },

  exportarRelatorioCSV: function () {
    const ganhosFiltrados = ganhosModule.getGanhosFiltrados();
    if (ganhosFiltrados.length === 0) return alert("Nenhum ganho para exportar.");
  ...
  compartilharWhatsApp: function () {
    ...
    const ganhosUsuario = storage.getGanhos().filter((g) => g.usuario === usuarioLogado.usuario);
    ...
    let msg = `Meu resumo do mês no MotoCash:\n\n` +
  ...
  compartilharGanhosFiltrados: function () {
    ...
    if (ganhosFiltrados.length === 0) {
      return alert("Nenhum ganho encontrado no período selecionado para compartilhar.");
    }
    
    const totais = ganhosFiltrados.reduce((acc, g) => ({
    ...
    if (select.value === "personalizado") {
      const inicio = new Date($("filtro-data-inicio").value + "T03:00:00").toLocaleDateString("pt-BR");
      const fim = new Date($("filtro-data-fim").value + "T03:00:00").toLocaleDateString("pt-BR");
